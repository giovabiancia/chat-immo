...
export default function ButtonLoadCoversation({ data, onConversationLoaded }) {
  ...
  const handleClick = () => {
    ...
    fetch(
      "http://152.42.137.28:1865/custom/get_conversation?chat_id=" + data.id,
      {
        method: "POST",
        headers: {
      ...
      .then((res) => res.json())
      .then((data) => {
        console.log("get_conversation ", data);
        let chat_history = data["messages"]["chat_history"];

        if (!chat_history) {
          return toast.warning(`Errore nel recupero della conversazione:`, {
            type: "warning",
          });
        }
